...
class Chatter:
    def __init__(self):
        ...
        self.count = 1
        self.entities_guide = json.load(open(Path('reference/entities_names.json'), 'r', encoding='utf8'))
        ...
        if os.path.isfile(Path('models/khwarizmi.h5')) and os.path.isfile(Path('models/khwarizmi.tkn')):
            self.model = load_model(str(Path('models/khwarizmi.h5')))
            self.tkn = pickle.load(open(Path('models/khwarizmi.tkn'), 'rb'))
            self.graph = get_default_graph()
        self.tagger = StanfordPOSTagger(str(Path('models/arabic.tagger')),
                                        path_to_jar=str(Path('models/stanford-postagger.jar')))
        self.required_entities = json.load(open(Path('reference/required_entities.json'), 'r', encoding='utf8'))
        self.required_entities_responses = json.load(open(Path('reference/required_entities_responses.json'),
                                                          'r', encoding='utf8'))
        self.default_value_entities = json.load(open(Path('reference/default_value_entities.json'),
                                                     'r', encoding='utf8'))
        chats = glob.glob(str(Path('intents/*.chat')))
        self.entity_intents = {}
        self.intents = {}
        for chat in chats:
    ...
    def train_intents(self):
        ...
        model.save(str(Path('models/khwarizmi.h5')))
        pickle.dump(tokenizer, open(Path('models/khwarizmi.tkn'), 'wb'))
        self.model = model
        self.tkn = tokenizer
        if os.path.isfile(Path('reference/khwarizmi.db')):
            os.remove(Path('reference/khwarizmi.db'))
        con = sqlite3.connect(str(Path('reference/khwarizmi.db')))
        c = con.cursor()
        c.execute(
    ...
    def predict_intent(self, sent):
        ...
        try:
            ...
            intent_con = sqlite3.connect(str(Path('reference/khwarizmi.db')))
    ...
    def train_entities(self):
        for intent in self.entity_intents.keys():
            ...
            try:
                ...
                print(intent.title())
                raise e
            pickle.dump(crf, open(Path('models/' + intent + '.sav'), 'wb'))
    ...
    def predict_entities(self, sent: str, intent):
        ...
        crf = pickle.load(open(Path('models/' + intent + '.sav'), 'rb'))
    ...
    def _resynonem(self, entity_name, entity_val, first=False):
        ...
        if name == 'any':
            ...
            entity_values = list(map(lambda x: x.split(','),
                                     open(Path('entities/' + name + '.ent'), 'r', encoding='utf8').read().split('\n')))
            ...
            if intent not in self.entity_intents.keys():
                try:
                    return {'intent': intent,
                            'res': choice(list(filter(bool, open(Path('responses/' + intent + '.res'),
                                               'r', encoding='utf8').read().split('\n'))))}
                except FileNotFoundError:
                    return {'intent': intent}
