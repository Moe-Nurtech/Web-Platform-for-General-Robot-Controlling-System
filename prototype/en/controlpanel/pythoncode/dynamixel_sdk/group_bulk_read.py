...
class GroupBulkRead:
    ...
    def txPacket(self):
        if len(list(self.data_dict.keys())) == 0:
        ...
        if self.ph.getProtocolVersion() == 1.0:
            return self.ph.bulkReadTx(self.port, self.param, len(list(self.data_dict.keys())) * 3)
        else:
            return self.ph.bulkReadTx(self.port, self.param, len(list(self.data_dict.keys())) * 5)
